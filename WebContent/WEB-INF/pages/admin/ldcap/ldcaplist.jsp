...
<title>Danh sách cấp cơ quan</title>
...
<script type="text/javascript">
	$(document).ready(function() {
			...
			if (list_activities_id == "") {
				...
				if (ok) {
					$.ajax({
						cache: false,
						type: 'POST',
						url:   '/admin/ldcap/delete.html',
						data:  'ldCapId=' + list_activities_id,
						dataType: "html",
						success: function(data) {
							...
							if ((getData.error != null) && (getData.error != '')) {
								...
								window.location.href = '/admin/ldcap/list.html'
							}
						},
		...
		$(".deleteIterm").click(function() {
			ok = confirm('Bạn chắc chắn muốn xóa?');
			if (ok)	{
				var ldCapId = this.id.substr(10);
				//alert(ldCapId);
				$.ajax({
					cache: false,
					type: 'POST',
					url:   '/admin/ldcap/delete.html',
					data:  'ldCapId=' + ldCapId,
					dataType: "html",
					success: function(data) {
						var getData = $.parseJSON(data);
						if ((getData.error != null) && (getData.error != '')) {
	...
	<div id="bodyadmin">
    	<div id="header">
            <div style="width: 250px; text-align:left; float:left; font-family: Tahoma; font-size:10pt;">
				...
				<div style="position:absolute; top:4px; left:14px;">&nbsp;Danh mục cấp cơ quan</div>				
            </div>
            <div style="width: 130px; text-align:right; float:right;">
            	<div style="position:absolute; top:1px;">
	                <a href="/admin/ldcap/add.html" title="News"><img id="btnadd" src="/admin/images/add.gif" style="border: none" title="New" /></a>&nbsp;
        ...
        <center>
        	<table id="tableOne" class="tablesorter" cellspacing="1" style="width:100%; padding-top: 0px; margin-top: 0px">
                ...
                <c:forEach var="ldcap" items="${listldcap}" varStatus="status">
					<tr>				
						<td align="right" style="padding-right: 4px;">
							<c:out value="${status.count}"/>
						</td>
						<td>
							<c:out value="${ldcap.getName()}"/>
						</td>
						<td align="center">
							<c:out value="${ldcap.isMacDinh()}"/>															
						</td>
                        <td>
                        	<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${ldcap.getCreated()}" />
                        </td>
                        <td>
                        	<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${ldcap.getModified()}" />
                        </td>
                        <td>
                            <span class="selectAllItems"><input id="checkbox-id-<c:out value="${ldcap.getLdCapId()}"/>" type="checkbox" name="chkdelete" title="Chọn" /></span>
                            <span class="deleteItems"><img src="/admin/images/delete.gif" class="deleteIterm" id="delete-id-<c:out value="${ldcap.getLdCapId()}"/>" title="Xóa" /></span>
                            <span class="editItems"><a href="/admin/ldcap/edit.html?ldCapId=<c:out value="${ldcap.getLdCapId()}"/>"><img src="/admin/images/lightning_go.gif" title="Sửa" class="editIterm" style="border: none" /></a></span>
						</td>
                    </tr>
                </c:forEach>
				</tbody>
