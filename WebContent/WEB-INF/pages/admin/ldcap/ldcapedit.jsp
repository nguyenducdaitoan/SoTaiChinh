...
<title>Chỉnh sửa cấp cơ quan</title>
	...
	$(document).ready(function() {
		$("img#imgSaveNews").click(function(){
			if ($("input[name='name']").val() == '') {
				alert('Bạn chưa nhập tên cấp cơ quan!');
				$('input[name="name"]').focus();
				$('input[name="name"]').select();
				return false;
			} 
			$("form[name=LdCapForm]").submit();	
		});
		$("form#formLdCapId").submit(function() {
			if ($("input[name='name']").val() == '') {
				alert('Bạn chưa nhập tên cấp cơ quan!');
				$('input[name="name"]').focus();
				$('input[name="name"]').select();
...
<body>
	<form:form commandName="ldcap" name="LdCapForm" id="formLdCapId" method="POST" action="/admin/ldcap/submitedit.html" accept-charset="UTF-8">
	<form:hidden path="ldCapId"/>
	<div id="bodyadmin">
	    <div id="header">
	        <div style="width: 250px; text-align:left; float:left; font-family: Tahoma; font-size:10pt;">
	            <img src="/admin/images/data.gif" title="" />&nbsp;Chỉnh sửa cấp cơ quan
	        </div>
