...
<title>Danh sách tin tức - sự kiện</title>
...
<script type="text/javascript">
	$(document).ready(function() {
			...
			if (list_activities_id == "") {
				...
				if (ok) {
					$.ajax({
						cache: false,
						type: 'POST',
						url:   '/admin/sukientintuc/delete.html',
						data:  'suKienTinTucId=' + list_activities_id,
						dataType: "html",
						success: function(data) {
							...
							if ((getData.error != null) && (getData.error != '')) {
								...
								window.location.href = '/admin/sukientintuc/list.html'
							}
						},
		...
		$(".deleteIterm").click(function() {
			ok = confirm('Bạn chắc chắn muốn xóa?');
			if (ok)	{
				var suKienTinTucId = this.id.substr(10);
				//alert(suKienTinTucId);
				$.ajax({
					cache: false,
					type: 'POST',
					url:   '/admin/sukientintuc/delete.html',
					data:  'suKienTinTucId=' + suKienTinTucId,
					dataType: "html",
					success: function(data) {
						var getData = $.parseJSON(data);
						if ((getData.error != null) && (getData.error != '')) {
	    ...
	    $("select[name='suKienId']").change(function() {
	    	if ($("select[name='suKienId']").val() != '') {
		    	window.location.href = "?suKienId=" + $("select[name='suKienId']").val();
		    }
		});
	});
</script>
...
<body>
	<div id="bodyadmin">
	<form:form commandName="sukien">
    	<div id="header">
            <div style="width: 250px; text-align:left; float:left; font-family: Tahoma; font-size:10pt;">
                <img src="/admin/images/data.gif" title="" />
				<div style="position:absolute; top:4px; left:14px;">&nbsp;Danh sách tin tức - sự kiện</div>				
            </div>
            <div style="width: 480px; text-align:right; float:right;">
            	<div style="position:absolute; top:1px;">
            		<form:select path="suKienId" name="suKienId" id="suKienId" style="background-color: #EDF8D6; border: 1px solid gray; font-family: Arial; width: 313px" class="SelectBox">
			         <form:option value="-1" label="------- Chọn một sự kiện --------"/>
				        <c:forEach items="${listSuKien}" var="sukien">       
				         <form:option value="${sukien.suKienId}" label="${sukien.name}"/>
				        </c:forEach>         
			       	</form:select> 
	                <a href="/admin/sukientintuc/add.html" title="News"><img id="btnadd" src="/admin/images/add.gif" style="border: none" title="New" /></a>&nbsp;
        ...
        <center>
        	<table id="tableOne" class="tablesorter" cellspacing="1" style="width:100%; padding-top: 0px; margin-top: 0px">
                ...
                <c:if test="${not empty listSuKienTinTuc}">
                <c:forEach var="SuKienTinTuc" items="${listSuKienTinTuc}" varStatus="status">
					<tr>				
						<td align="right" style="padding-right: 4px;"><c:out value="${status.count}" /></td>
						<td><c:out value="${SuKienTinTuc.title}" /></td>
						<td>
							<c:if test="${SuKienTinTuc.active}">Active</c:if>
							<c:if test="${!SuKienTinTuc.active}">Deactive</c:if>							
						</td>  
                        <td><fmt:formatDate pattern="dd/MM/yyyy HH:mm" value="${SuKienTinTuc.modified}" /></td>
                        <td>
                            <span class="selectAllItems"><input id="checkbox-id-<c:out value="${SuKienTinTuc.suKienTinTucId}" />" type="checkbox" name="chkdelete" title="Chọn" /></span>
                            <span class="deleteItems"><img src="/admin/images/delete.gif" class="deleteIterm" id="delete-id-<c:out value="${SuKienTinTuc.suKienTinTucId}" />" title="Xóa" /></span>
                            <span class="editItems"><a href="/admin/sukientintuc/edit.html?suKienTinTucId=<c:out value="${SuKienTinTuc.suKienTinTucId}" />"><img src="/admin/images/lightning_go.gif" title="Sửa" class="editIterm" style="border: none" /></a></span>
						</td>
                    </tr>
                </c:forEach>
                </c:if>
