...
<script type="text/javascript">
	$(document).ready(function() {
		$("#btndelete").click(function() {
			...
			if (list_activities_id == "") {
				alert('Hãy chọn một hình ảnh để xóa');
			} else {
				...
				if (ok) {
					$.ajax({
						cache: false,
						type: 'GET',
						url:   '/admin/hinhanh/delete.html',
						data:  'hinhAnhId=' + list_activities_id,
						dataType: "html",
						success: function(data) {
							...
							if ((getData.error != null) && (getData.error != '')) {
								...
								window.location.href = '/admin/hinhanh/list.html'
							}
						},
		...
		$(".deleteIterm").click(function() {
			...
			ok = confirm('Bạn chắc chắn muốn xóa?');
			if (ok)	{
				var hinhanhId = this.id.substr(10);
				alert(hinhanhId);
				//alert(hinhanhId);
				$.ajax({
					cache: false,
...
*/		$("select[name='page']").change(function(){
			window.location.href = "/admin/hinhanh/list.html?page=" + $("select[name='page']").val();
		});
	});
</script>
</head>
	...
	<div id="bodyadmin">
    	<div id="header">
            <div style="width: 250px; text-align:left; float:left; font-family: Tahoma; font-size:10pt;">
				...
				<div style="position:absolute; top:4px; left:14px;">&nbsp;Danh sách hình ảnh</div>				
            </div>
            <div style="width: 130px; text-align:right; float:right;">
            	<div style="position:absolute; top:1px;">
	                <a href="/admin/hinhanh/add.html" title="News"><img id="btnadd" src="/admin/images/add.gif" style="border: none" title="New" /></a>&nbsp;
        ...
        <center>
        	<table id="tableOne" class="tablesorter" cellspacing="1" style="width:100%; padding-top: 0px; margin-top: 0px">
                ...
                <c:if test="${not empty listHinhAnh}">
                <c:forEach var="hinhanh" items="${listHinhAnh}" varStatus="status">
					<tr>				
						<td align="right" style="padding-right: 4px;"><c:out value="${status.count}" /></td>
						<td width="60px" align="center"><img src="<c:url value='${hinhanh.path}' />" width="50" height="50" alt="Your Image" align="center" /></td>
						<td><c:out value="${hinhanh.title}" /></td>
                        <td><fmt:formatDate pattern="dd/MM/yyyy HH:mm" value="${hinhanh.created}" /></td>
                        <td><fmt:formatDate pattern="dd/MM/yyyy HH:mm" value="${hinhanh.modified}" /></td>
                        <td>
                           <span class="selectAllItems"><input id="checkbox-id-<c:out value="${hinhanh.hinhAnhId}" />" type="checkbox" name="chkdelete" title="Chọn" /></span>
                            <span class="deleteItems"><img src="/admin/images/delete.gif" class="deleteIterm" id="delete-id-<c:out value="${hinhanh.hinhAnhId}" />" title="Xóa" /></span>
                            <span class="editItems"><a href="/admin/hinhanh/edit.html?hinhanhId=<c:out value="${hinhanh.hinhAnhId}" />"><img src="/admin/images/lightning_go.gif" title="Sửa" class="editIterm" style="border: none" /></a></span>
						</td>
                    </tr>
                </c:forEach>
                </c:if>
