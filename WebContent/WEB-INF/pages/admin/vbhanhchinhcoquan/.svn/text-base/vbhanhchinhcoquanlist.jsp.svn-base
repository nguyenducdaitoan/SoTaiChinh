...
<script type="text/javascript">
	$(document).ready(function() {
		$("#btndelete").click(function() {
			...
			if (list_activities_id == "") {
				alert('Hãy chọn một cấp cơ quan để xóa');
			} else {
				...
				if (ok) {
					$.ajax({
						cache: false,
						type: 'POST',
						url:   '/admin/vbhanhchinhcoquan/deleteajaxldcoquan.html',
						data:  'ldCoQuanId=' + list_activities_id,
						dataType: "html",
						success: function(data) {
							...
							if ((getData.error != null) && (getData.error != '')) {
								...
								window.location.href = '/admin/vbhanhchinhcoquan/list.html?type=2'
							}
						},
		...
		$(".deleteIterm").click(function() {
			ok = confirm('Bạn chắc chắn muốn xóa?');
			if (ok)	{
				var ldCoQuanId = this.id.substr(10);
				$.ajax({
					cache: false,
					type: 'POST',
					url:   '/admin/vbhanhchinhcoquan/deleteajaxldcoquan.html',
					data:  'ldCoQuanId=' + ldCoQuanId,
					dataType: "html",
					success: function(data) {
						var getData = $.parseJSON(data);
						if ((getData.error != null) && (getData.error != '')) {
	...
	<div id="bodyadmin">
    	<div id="header">
            <div style="width: 250px; text-align:left; float:left; font-family: Tahoma; font-size:10pt;">
				...
				<div style="position:absolute; top:4px; left:14px;">&nbsp;Danh mục cơ quan</div>				
            </div>
            <div style="width: 130px; text-align:right; float:right;">
            	<div style="position:absolute; top:1px;">
	                <a href="/admin/vbhanhchinhcoquan/add.html?type=2" title="News"><img id="btnadd" src="/admin/images/add.gif" style="border: none" title="New" /></a>&nbsp;
        ...
        <center>
        	<table id="tableOne" class="tablesorter" cellspacing="1" style="width:100%; padding-top: 0px; margin-top: 0px">
                ...
                <c:forEach var="ldcoquan" items="${listldcoquan}" varStatus="status">
					<tr>				
						<td align="right" style="padding-right: 4px;">
							<c:out value="${status.count}"/>
						</td>
						<td>
							<c:out value="${ldcoquan.getName()}"/>
						</td>
						<td>
							<c:out value="${ldcoquan.getTblLdCap().getName()}"/>
						</td>						
                        <td>
                        	<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${ldcoquan.getCreated()}" />
                        </td>
                        <td>
                        	<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${ldcoquan.getModified()}" />
                        </td>
                        <td>
                            <span class="selectAllItems"><input id="checkbox-id-<c:out value="${ldcoquan.getLdCoQuanId()}"/>" type="checkbox" name="chkdelete" title="Chọn" /></span>
                            <span class="deleteItems"><img src="/admin/images/delete.gif" class="deleteIterm" id="delete-id-<c:out value="${ldcoquan.getLdCoQuanId()}"/>" title="Xóa" /></span>
                            <span class="editItems"><a href="/admin/vbhanhchinhcoquan/edit.html?ldCoQuanId=<c:out value="${ldcoquan.getLdCoQuanId()}"/>"><img src="/admin/images/lightning_go.gif" title="Sửa" class="editIterm" style="border: none" /></a></span>
						</td>
                    </tr>
 				</c:forEach>
				</tbody>
