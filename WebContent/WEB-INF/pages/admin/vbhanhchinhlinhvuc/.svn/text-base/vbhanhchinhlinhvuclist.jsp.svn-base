...
<title>Danh sách lĩnh vực văn bản</title>
...
<script type="text/javascript">
	$(document).ready(function() {
		$("#btndelete").click(function() {
			...
			if (list_activities_id == "") {
				alert('Hãy chọn một lĩnh vực để xóa');
			} else {
				...
				if (ok) {
					$.ajax({
						cache: false,
						type: 'POST',
						url:   '/admin/vbhanhchinhlinhvuc/delete.html',
						data:  'ldLinhVucId=' + list_activities_id,
						dataType: "html",
						success: function(data) {
							...
							if ((getData.error != null) && (getData.error != '')) {
								...
								window.location.href = '/admin/vbhanhchinhlinhvuc/list.html?type=2';
							}
						},
		...
		$(".deleteIterm").click(function() {
			ok = confirm('Bạn chắc chắn muốn xóa?');
			if (ok)	{
				var ldLinhVucId = this.id.substr(10);
				//alert(ldLinhVucId);
				$.ajax({
					cache: false,
					type: 'POST',
					url:   '/admin/vbhanhchinhlinhvuc/delete.html',
					data:  'ldLinhVucId=' + ldLinhVucId,
					dataType: "html",
					success: function(data) {
						var getData = $.parseJSON(data);
						if ((getData.error != null) && (getData.error != '')) {
	...
	<div id="bodyadmin">
    	<div id="header">
            <div style="width: 250px; text-align:left; float:left; font-family: Tahoma; font-size:10pt;">
				...
				<div style="position:absolute; top:4px; left:14px;">&nbsp;Danh mục lĩnh vực văn bản</div>				
            </div>
            <div style="width: 130px; text-align:right; float:right;">
            	<div style="position:absolute; top:1px;">
	                <a href="/admin/vbhanhchinhlinhvuc/add.html?type=1" title="News"><img id="btnadd" src="/admin/images/add.gif" style="border: none" title="New" /></a>&nbsp;
        ...
        <center>
        	<table id="tableOne" class="tablesorter" cellspacing="1" style="width:100%; padding-top: 0px; margin-top: 0px">
                <thead>
                    <tr>
                    	<th width="30px">#</th>
                        <th>Tên lĩnh vực văn bản</th>
                        <th width="80px">Mặc định</th>
                ...
                <tbody style="text-align:left;">
					...
					<c:forEach var="ldlinhvuc" items="${listldlinhvuc}" varStatus="status">
					<tr>				
						<td align="right" style="padding-right: 4px;">
							<c:out value="${status.count}"/>
						</td>
						<td>
							<c:out value="${ldlinhvuc.getName()}"/>
						</td>
						<td align="center">
							
							<c:choose>
								<c:when test="${ldlinhvuc.isMacDinh()==false }">
									<c:out value="Ẩn"/>
								</c:when>
								<c:otherwise>
									<c:out value="Hiện"/>
                        ...
                        <td>
                        	<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${ldlinhvuc.getCreated()}" />
                        </td>
                        <td>
                        	<fmt:formatDate pattern="yyyy-MM-dd HH:mm" value="${ldlinhvuc.getModified()}" />
                        </td>
                        <td>
                            <span class="selectAllItems"><input id="checkbox-id-<c:out value="${ldlinhvuc.getLdLinhVucId()}"/>" type="checkbox" name="chkdelete" title="Chọn" /></span>
                            <span class="deleteItems"><img src="/admin/images/delete.gif" class="deleteIterm" id="delete-id-<c:out value="${ldlinhvuc.getLdLinhVucId()}"/>" title="Xóa" /></span>
                            <span class="editItems"><a href="/admin/vbhanhchinhlinhvuc/edit.html?ldLinhVucId=<c:out value="${ldlinhvuc.getLdLinhVucId()}"/>"><img src="/admin/images/lightning_go.gif" title="Sửa" class="editIterm" style="border: none" /></a></span>
						</td>
                    </tr>
                </c:forEach>
