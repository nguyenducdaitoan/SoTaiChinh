			...
			if (list_activities_id == "") {
				...
				if (ok) {
					$.ajax({
						cache: false,
						type: 'POST',
						url:   '/admin/users/delete.html',
						data:  'usersId=' + list_activities_id,
						dataType: "html",
    	...
    	<div id="header">
            ...
            <div style="width: 130px; text-align:right; float:right;">
            	<div style="position:absolute; top:1px;">
	                <a href="/admin/users/add.html" title="News"><img id="btnadd" src="/admin/images/add.gif" style="border: none" title="New" /></a>&nbsp;
        	...
        	<table id="tableOne" class="tablesorter" cellspacing="1" style="width:100%; padding-top: 0px; margin-top: 0px">
                ...
                <c:forEach var="users" items="${listUsers}" varStatus="status">
					<tr>				
						...
						<td>
							<c:if test="${users.active}">Active</c:if>
							<c:if test="${!users.active}">Deactive</c:if>							
						</td>
                        <td><fmt:formatDate pattern="dd/MM/yyyy HH:mm" value="${users.created}" /></td>
                        <td><fmt:formatDate pattern="dd/MM/yyyy HH:mm" value="${users.modified}" /></td>
                        <td>
                            <span class="selectAllItems"><input id="checkbox-id-<c:out value="${users.usersId}" />" type="checkbox" name="chkdelete" title="Chọn" /></span>
                            <span class="deleteItems"><img src="/admin/images/delete.gif" class="deleteIterm" id="delete-id-<c:out value="${users.usersId}" />" title="Xóa" /></span>
                            <span class="editItems"><a href="/admin/users/edit.html?usersId=<c:out value="${users.usersId}" />"><img src="/admin/images/lightning_go.gif" title="Sửa" class="editIterm" style="border: none" /></a></span>
						</td>
                    </tr>
                </c:forEach>
                </c:if>
