...
<script type="text/javascript" charset="utf-8">
	...
	$(document).ready(
			function() {
				...
				$("#btndelete").click(function() {
					...
					if (list_activities_id == "") {
						alert('Hãy chọn một chuyên mục tin tức để xóa');
					} else {
						...
						if (ok) {
							$.ajax({
								cache: false,
								type: 'POST',
								url:   '/admin/tintuc/deleteAjax.html',
								cache: false,						
								data:  'chuyenMucId=' + list_activities_id,
								dataType: "html",
								success: function(data) {
									...
									if ((getData.error != null) && (getData.error != '')) {
										...
										window.location.href = '/admin/tintuc/danhmuclist.html';
									}
								},
				...
				$(".deleteIterm").click(function() {
					...
					if (ok)	{
						var chuyenMucId = this.id.substr(10);
						$.ajax({
							cache: false,
							type: 'POST',
							url:   '/admin/tintuc/deleteAjax.html',
							cache: false,
							data:  'chuyenMucId=' + chuyenMucId,
							dataType: "html",
							success: function(data) {
								var getData = $.parseJSON(data);
								if ((getData.error != null) && (getData.error != '')) {
				...
				$("[name='danhmucForm']").submit(function() {
					...
					if ($("input:hidden[name='action']").val() != 'delete') {
						...
						if ($("input:text[name='priority']").val() == '') {
							alert('Chưa nhập thứ tự');
							$("input:text[name='priority']").focus();
							return false;
						}
						var priority = $("input:text[name='priority']").val();
						//alert(priority);
						if(priority > 100) {
							alert('Nhập thứ tự bị sai, vui lòng nhập lại !');
							return false;
						}
					}
				...
				$("input:button[name='reset']").click(function() {
					$("input:hidden[name='chuyenMucId']").val("");
				...
				$("input:button[name='delete']").click(function() {
					...
					if ($("input:hidden[name='chuyenMucId']").val() == '') {
						alert('chuyên mục tin tức không tồn tại. Chọn chuyên mục tin tức để xóa');
						$("input:hidden[name='chuyenMucId']").focus();
						return false;
					}
					ok = confirm('Bạn chắc chắn muốn xóa?');
					if(ok){
	...
	function fillThongDanhMuc(chuyenMucId, name, priority, active) {
		...
		$("input:hidden[name='chuyenMucId']").val(chuyenMucId);
		$("input:text[name='title']").val(name);
		...
		$("tr#row-" + chuyenMucId).find('td').each(function() {
			$(this).css("background-color", "yellow");
		});
		show_hide_botton_del(true);
		lastClicksukienId = chuyenMucId;
	}
	...
	<div id="bodyadmin">
		<div id="header">
			<div
				...
				<div style="position: absolute; top: 4px; left: 14px;">&nbsp;Chuyên mục tin tức</div>
			</div>
		...
		<div id="content"
			...
			<form:form name="danhmucForm" method="POST" commandName="danhmuc" action="/admin/tintuc/action.html"
				accept-charset="UTF-8">
				<form:hidden path="chuyenMucId"/> 
				...
				<div style="padding: 10px 0px;">
					...
					<div>
						<div
							style="float: left; padding-top: 1px; width: 140px; text-align: right; padding-right: 3px; font-family: Arial; font-size: 10pt;">
							<span style="color: red; font-style: italic;">(*)</span>Thứ tự&nbsp;:
						</div>
						<div style="float: left; width: 483px; text-align: left;">
							<form:input  path="priority" maxlength="8" style="width: 6%;" cssClass="textbox"/>
						</div>
					</div>
					<div style="clear: both; height: 3px;"></div>
					<div
...
%>
					...
					<tr
						onclick="javascript: fillThongDanhMuc('<c:out value="${tbldanhmuc.getChuyenMucId()}"/>', '<c:out value="${tbldanhmuc.getTitle()}"/>', '<c:out value="${tbldanhmuc.getPriority()}"/>');"
						id="row-<c:out value="${tbldanhmuc.getChuyenMucId()}"/>" style="cursor: pointer;">
						<td align="right" style="padding-right: 4px;" id="30"><%=iSTT %></td>
						<td align="left"><c:out value="${tbldanhmuc.getTitle()}"/></td>
						<td>
							<span class="selectAllItems">
								<input	type="checkbox" title="Chọn" name="chkdelete"id="checkbox-id-<c:out value="${tbldanhmuc.getChuyenMucId()}"/>"></span> 
								<span class="deleteItems"><img	title="Xóa" id="delete-id-<c:out value="${tbldanhmuc.getChuyenMucId()}"/>" class="deleteIterm" src="/admin/images/delete.gif">
							</span>
						</td>
					</tr>
<%
