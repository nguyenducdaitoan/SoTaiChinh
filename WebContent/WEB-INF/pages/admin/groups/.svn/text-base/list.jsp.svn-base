...
<script type="text/javascript">
	$(document).ready(function() {
			...
			if (list_activities_id == "") {
				...
				if (ok) {
					$.ajax({
						cache: false,
						type: 'POST',
						url:   '/admin/groups/delete.html',
						data:  'groupsId=' + list_activities_id,
						dataType: "html",
						success: function(data) {
							...
							if ((getData.error != null) && (getData.error != '')) {
								...
								window.location.href = '/admin/groups/list.html'
							}
						},
		...
		$(".deleteIterm").click(function() {
			ok = confirm('Bạn chắc chắn muốn xóa?');
			if (ok)	{
				var groupsId = this.id.substr(10);
				$.ajax({
					cache: false,
					type: 'POST',
					url:   '/admin/groups/delete.html',
	...
	<div id="bodyadmin">
    	<div id="header">
            <div style="width: 250px; text-align:left; float:left; font-family: Tahoma; font-size:10pt;">
				...
				<div style="position:absolute; top:4px; left:14px;">&nbsp;Danh sách người dùng</div>				
            </div>
            <div style="width: 130px; text-align:right; float:right;">
            	<div style="position:absolute; top:1px;">
	                <a href="/admin/groups/add.html" title="News"><img id="btnadd" src="/admin/images/add.gif" style="border: none" title="New" /></a>&nbsp;
        ...
        <center>
        	<table id="tableOne" class="tablesorter" cellspacing="1" style="width:100%; padding-top: 0px; margin-top: 0px">
                ...
                <c:forEach var="groups" items="${listGroups}" varStatus="status">
					<tr>				
						<td align="right" style="padding-right: 4px;"><c:out value="${status.count}" /></td>
						<td><c:out value="${groups.name}" /></td>
                        <td><fmt:formatDate pattern="dd/MM/yyyy HH:mm" value="${groups.created}" /></td>
                        <td><fmt:formatDate pattern="dd/MM/yyyy HH:mm" value="${groups.modified}" /></td>
                        <td>
                            <span class="selectAllItems"><input id="checkbox-id-<c:out value="${groups.groupsId}" />" type="checkbox" name="chkdelete" title="Chọn" /></span>
                            <span class="deleteItems"><img src="/admin/images/delete.gif" class="deleteIterm" id="delete-id-<c:out value="${groups.groupsId}" />" title="Xóa" /></span>
                            <span class="editItems"><a href="/admin/groups/edit.html?groupsId=<c:out value="${groups.groupsId}" />"><img src="/admin/images/lightning_go.gif" title="Sửa" class="editIterm" style="border: none" /></a></span>
						</td>
                    </tr>
                </c:forEach>
                </c:if>
