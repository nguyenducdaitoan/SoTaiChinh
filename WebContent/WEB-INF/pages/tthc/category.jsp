...
<c:choose>
	<c:when test="${empty listThuTucByChuyenMuc}">
	...
	<c:otherwise>
		<c:forEach var="thuTucHanhChinh" items="${ listThuTucByChuyenMuc}" >
			<c:set var="lastChuyenMucId" value="${thuTucHanhChinh.tblTthcDm.tthcDmId }"/>
		<c:if test="${(chuyenMucId == 0) || ((chuyenMucId != 0) && (chuyenMucId != lastChuyenMucId)) }">
			...
			<div class="box_content have_title">
				<h1 class="title_content title_content_position">
					<a href="tthc/category.html?tthcDmId=<c:out value="${thuTucHanhChinh.tblTthcDm.tthcDmId}"/>">
					<span class="title_span_left"></span>
					<span class="title_span_center">
						<c:out value="${thuTucHanhChinh.tblTthcDm.name}"/>
					</span>
					<span class="title_span_right"></span>
					</a>
				</h1>
				<div class="info_box_cotent">
					...
					<div class="box_content_post box_content_post_tthc">
						<div style="padding: 1px 14px 2px 14px; text-align: left; font-size: 10pt;">
							<a class="linkbmdt" style="background: url('/images/arrow_menu.gif') no-repeat scroll 0 2px transparent; padding-left: 15px" href="/tthc/detail.html?tthcId=<c:out value="${thuTucHanhChinh.tthcId}"/>"><c:out value="${thuTucHanhChinh.title}"/></a><span class="date_content">(<fmt:formatDate pattern="dd/MM/yyyy" timeZone="GMT+7" value="${thuTucHanhChinh.modified }"/>)</span>
						</div>
					</div>	
