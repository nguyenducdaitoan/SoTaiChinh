...
<c:choose>
	...
	<c:otherwise>
		<c:forEach var="bmdt" items="${ listbmdt}" >
			<c:set var="lastChuyenMucId" value="${bmdt.tblBmdtDm.bmdtDmId }"/>
		<c:if test="${(chuyenMucId == 0) || ((chuyenMucId != 0) && (chuyenMucId != lastChuyenMucId)) }">
			...
			<div class="box_content have_title">
				<h1 class="title_content title_content_position">
					<a href="bmdt/category.html?tthcDmId=<c:out value="${bmdt.tblBmdtDm.bmdtDmId}"/>">
					<span class="title_span_left"></span>
					<span class="title_span_center">
						<c:out value="${bmdt.tblBmdtDm.name}"/>
					</span>
					<span class="title_span_right"></span>
					</a>
				</h1>
				<div class="info_box_cotent">
					...
					<div class="box_content_post box_content_post_tthc">
						<div style="padding: 1px 14px 2px 14px; text-align: left; font-size: 10pt;">
							<a class="linkbmdt" style="background: url('/images/arrow_menu.gif') no-repeat scroll 0 2px transparent; padding-left: 15px" href="/bmdt/detail.html?bmdtId=<c:out value="${bmdt.bmdtId}"/>"><c:out value="${bmdt.description}"/></a><span class="date_content">(<fmt:formatDate pattern="dd/MM/yyyy" timeZone="GMT+7" value="${bmdt.modified }"/>)</span>
						</div>
					</div>	
